...
@section('content')
    ...
    <div class="border-gray-600 bg-white rounded m-4 p-4">
        <section id="header" class="flex w-full divide-x-2 font-bold">
            <form action="{{ route('admin.menus.store') }}" method="POST" class="w-full">
                @csrf
                <div class="flex my-4">
                    <label for="title" class="w-1/3">Title</label>
                    <input type="text" name="title" id="title" value="{{ old('title') }}" class="border-gray-400 rounded px-4 py-2 flex-1">
                </div>
                <div class="flex my-4">
                    <label for="description" class="w-1/3">Description</label>
                    <textarea name="description" id="description" class="border-gray-400 rounded px-4 py-2 flex-1">{{ old('description') }}</textarea>
                </div>
                <div class="flex justify-end">
                    <button class="rounded bg-blue-500 px-4 py-2 text-white">SAVE NEW MENU</button>
                </div>
            </form>
        </section>
    </div>
