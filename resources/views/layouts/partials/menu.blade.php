<nav class="fixed top-0 left-0 bottom-0 w-64 bg-blueGray-700 text-white flex flex-col justify-between">
    <ul>
        <li class="text-2xl font-bold text-center border-b-2 border-white mt-4">{{ config('app.name', 'Laravel') }}</li>
        @can('access-admin')
            <x-menu.item route="{{ route('admin.dashboard') }}" title="Admin Panel" class="mt-4 border-t-2 border-b-2 border-blueGray-500"></x-menu.item>
        @endcan
    ...
    <section class="d-flex flex-col border-t-2 border-white">
        ...
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button class="py-2 w-full hover:bg-blueGray-500">{{__('Logout')}}</button>
        </form>
    </section>
    @else
        <ul>
